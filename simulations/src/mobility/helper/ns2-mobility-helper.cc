          ...
          if (IsSetInitialPos (pr))
          ...
          else // NOW EVENTS TO BE SCHEDULED
            {
              ...
              if (IsSchedMobilityPos (pr))
                {
            	  //FIXME: BEGIN Workaround (to get the first time (in s) of each node)
				  if (at > 0 && iNodeId >= 0 && m_nodeFirstTime.find(iNodeId) == m_nodeFirstTime.end())
				  {
					  m_nodeFirstTime[iNodeId] = at;
				  }
				  // END Workaround
...
double Ns2MobilityHelper::getNodeIdFirstTime(int nodeId)
{
	double ret = 0;
	std::map<int, double>::iterator it = m_nodeFirstTime.find(nodeId);

	if ( it != m_nodeFirstTime.end())
	{
		ret = it->second;
	}
	return ret;
